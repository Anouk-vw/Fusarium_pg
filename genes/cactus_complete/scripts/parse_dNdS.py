...
def loop_codeml(in_dir):
    ...
    for in_file in os.listdir(in_dir):
        ...
        if omega_list:
            ...
            results_dict[OG.replace('.txt','')] = [np.mean(omega_list), np.median(omega_list)]
    return results_dict
...
if __name__ == '__main__':
    with open('dnds_per_OG_2.txt', 'w+') as dnds_out:
        dnds_out.write(f"OG\tmean_dNdS\tmedian_dNdS\n")
        for k,v in loop_codeml(argv[1]).items():
            dnds_out.write(f"{k}\t{v[0]}\t{v[1]}\n")
